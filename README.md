...
A single shared **Discover** library — movies and shows sit as direct siblings in one flat folder (`CollectionTypeOptions.mixed`; distinguished internally by structure — a show folder always has a `tvshow.nfo`, a movie folder never does — rather than separate subfolders, so nothing sits behind an extra folder click) — visible to every user, populated with trending/popular content. Unlike AI Picks, this isn't personalized per user and doesn't call the LLM at all; it's a lightweight "what's popular right now" feed, closer to what a plain Jellyseerr/TMDB discover browse looks like.
